...
def test_create_room_returns_identity_and_token(broker):
    ...
    assert result["join_token"].startswith(result["room_id"] + ".")
...
def test_room_ids_and_tokens_are_unique_and_unguessable(broker):
    ...
    secrets = {r["join_token"].split(".", 1)[1] for r in rooms}
...
def test_join_tokens_are_stored_hashed_not_plaintext(broker, workspace):
    ...
    secret = result["join_token"].split(".", 1)[1]
...
def test_participant_secrets_are_stored_hashed(broker, workspace):
    result = broker.create_room("Room")
    joined = broker.join(result["join_token"], "claude")
    assert joined["secret"].encode() not in workspace.db_path.read_bytes()


def test_wrong_room_token_is_rejected(broker):
    room_a = broker.create_room("A")
    room_b = broker.create_room("B")
    secret_b = room_b["join_token"].split(".", 1)[1]

    with pytest.raises(AuthError):
        broker.join(room_a["room_id"], "claude", token=secret_b)
...
def test_observer_can_read_with_the_room_token_only(room):
    ...
    result = room.broker.read(room.room_id, credential=room.observer_token())
...
def test_stopped_room_rejects_new_joins(room):
    ...
    with pytest.raises(StateError):
        room.broker.join(room.join_token, "gemini")
