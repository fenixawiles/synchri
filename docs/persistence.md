...
The most common misreading of Synchri is that a room is a chat that disappears
when you close the terminal. It is not. Everything Synchri owns is durable from
the first write, because there is no in-memory broker for state to live in.

| Thing | Where it lives | Survives the session? | Owner |
|---|---|---|---|
| Transcript | `messages` table + `transcript.jsonl` | **yes** | Synchri |
| Shared memory ledger | `rooms/<id>/memory.md` | **yes** | Synchri |
| Queue, turns, tasks, active speaker | SQLite | **yes** | Synchri |
| Provenance and audit events | SQLite | **yes** | Synchri |
| Participant identities and invites | SQLite | **yes** | Synchri |
| **An agent's own reasoning and working context** | its own platform | **only if the agent saves it** | **the agent** |

That last row is the deliberate boundary. Synchri is not a memory system for
...
```console
$ rm ~/.synchri/current_room      # forget everything about which room it was
...
Room resolution order is: `--room`, then `$SYNCHRI_ROOM`, then **the newest
...
`join` returns and prints a briefing; `synchri briefing --as <name>` re-fetches
...
- **The repo check is advisory.** A mismatch is a loud warning in the briefing,
  not a refusal. Synchri does not control what an agent edits, so blocking the
...
- **The ledger is not summarized.** It grows until someone prunes it
  (`synchri memory remove <section> <n>`); only handoffs roll off automatically.
